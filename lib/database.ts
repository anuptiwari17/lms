...
export const announcementQueries = {
  async getByCourse(courseId: string): Promise<(Announcement & { created_by_name: string })[]> {
    const { data, error } = await supabase
      .from('announcements')
      .select(`
        id,
      ...
      `)
      .eq('course_id', courseId)
      .order('created_at', { ascending: false }) as { data: AnnouncementWithUser[] | null, error: unknown }
    ...
    return (data || []).map(ann => ({
      ...
      users: ann.users,
      created_by_name: ann.users?.name ?? 'Unknown'
    }))
  },

  async getById(id: string): Promise<Announcement & { created_by_name: string } | null> {
    const { data, error } = await supabase
      .from('announcements')
      .select(`
        id,
      ...
      `)
      .eq('id', id)
      .single() as { data: AnnouncementWithUser | null, error: unknown }
  ...
  async create(announcementData: CreateAnnouncementData): Promise<Announcement & { created_by_name: string } | null> {
    if (!announcementData.content.trim()) {
      throw new Error('Announcement content cannot be empty')
    }
    if (announcementData.content.length > 5000) {
  ...
  async update(id: string, updates: UpdateAnnouncementData): Promise<Announcement & { created_by_name: string } | null> {
